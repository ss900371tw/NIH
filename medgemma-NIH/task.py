...
def create_model_and_tokenizer1():
...
def create_model_and_tokenizer():
    ...
    # 直接使用包含 LoRA 的 Repo
    model_name = "ss900371tw/medgemma-NIH-lora"
    print(f"🚀 [Task - GPU 專用模式] 正在透過 Unsloth 載入現有 Checkpoint 模型 {model_name}...")
    import unsloth
    from unsloth import FastVisionModel
    
    force_clear_gpu_cache()
    
    # from_pretrained 會自動載入 Base Model 並將你的 LoRA adapter 套用上去
    model, processor = FastVisionModel.from_pretrained(
        model_name=model_name,
        load_in_4bit=True,
        attn_implementation="sdpa", 
    ...
    # ⚠️ 不需要再執行 FastVisionModel.get_peft_model() !
    return model, processor
...
def prepare_vqa_dataset(
    csv_path, image_root, num, is_train=True, current_seed=None, seed=42
):
    ...
    if img_col is None:
        raise ValueError(
            f"❌ [CSV 欄位錯誤] 在 {csv_path} 中找不到任何圖片欄位！現有欄位為: {df.columns.tolist()}"
        )
    ...
    # 決定當前抽樣使用的 Seed
    sampling_seed = (
        seed if (not is_train or current_seed is None) else current_seed
    )

    local_random = random.Random(sampling_seed)
    abnormal_ratio = local_random.uniform(0.65, 0.7)

    n_healthy_target = min(
        int(total_requested * (1 - abnormal_ratio)), len(df_healthy)
    )
    n_abnormal_target = min(
        total_requested - n_healthy_target, len(df_abnormal)
    )
    n_healthy_target = min(
        total_requested - n_abnormal_target, len(df_healthy)
    )

    disease_labels = [
        l for l in target_labels if l in df.columns and l != "No_Finding"
    ]

    if not is_train:
        # 🧪 測試/驗證集：從全體異常資料中，依疾病陽性總數由少到多（稀有優先）進行動態抽樣
        print(
            f"\n🧪 [測試/驗證集 - 全域稀有優先抽樣] 使用 Seed: {sampling_seed}"
        )
        guaranteed_abnormal_indices = set()

        while True:
            still_needed = n_abnormal_target - len(
                guaranteed_abnormal_indices
            )
            if still_needed <= 0:
                break

            # 取得尚未被抽取的剩餘異常資料池
            df_abnormal_remaining = df_abnormal[
                ~df_abnormal.index.isin(guaranteed_abnormal_indices)
            ]
            if df_abnormal_remaining.empty:
                print("  └ 異常資料池已全部抽取完畢。")
                break

            # 統計剩餘異常池中，各疾病標籤出現的陽性總數
            class_counts = {
                l: (df_abnormal_remaining[l] == 1).sum() for l in disease_labels
            }
            ...
            df_min_label_samples = df_abnormal_remaining[
                df_abnormal_remaining[min_label] == 1
            ]
            ...
            if available_count <= still_needed:
                guaranteed_abnormal_indices.update(
                    df_min_label_samples.index.tolist()
                )
                print(
                    f"  └ 稀有標籤 [{min_label}] (剩餘可用 {available_count} 筆) -> 全抽，累積異常: {len(guaranteed_abnormal_indices)}/{n_abnormal_target}"
                )
            else:
                sampled = df_min_label_samples.sample(
                    n=still_needed, random_state=sampling_seed
                )
                guaranteed_abnormal_indices.update(sampled.index.tolist())
                print(
                    f"  └ 稀有標籤 [{min_label}] (剩餘可用 {available_count} 筆) -> 隨機抽取 {still_needed} 筆補滿，達目標額度"
                )
                break
        ...
        if still_needed > 0:
            df_remaining = df_abnormal[
                ~df_abnormal.index.isin(guaranteed_abnormal_indices)
            ]
            if not df_remaining.empty:
                sampled_backup = df_remaining.sample(
                    n=min(still_needed, len(df_remaining)),
                    random_state=sampling_seed,
                )
                guaranteed_abnormal_indices.update(
                    sampled_backup.index.tolist()
                )
        ...
        if n_healthy_target > 0 and len(df_healthy) > 0:
            final_healthy = df_healthy.sample(
                n=n_healthy_target, random_state=sampling_seed
            )
            df_balanced = (
                pd.concat([final_abnormal, final_healthy])
                .sample(frac=1, random_state=sampling_seed)
                .reset_index(drop=True)
            )
        else:
            df_balanced = final_abnormal.sample(
                frac=1, random_state=sampling_seed
            ).reset_index(drop=True)

    else:
        # 🏋️ 訓練集：多疾病最少疾病優先，並依條件聯動單一疾病群
        print(f"\n🏋️ [訓練集 - 動態聯動抽樣] 使用 Seed: {sampling_seed}")
        guaranteed_abnormal_indices = set()
        ...
        while True:
            ...
            # 取得多疾病群中尚未被抽取的剩餘樣本
            df_multi_remaining = df_multi[
                ~df_multi.index.isin(guaranteed_abnormal_indices)
            ]
            ...
            # 統計多疾病群中各標籤的剩餘數量
            class_counts_multi = {
                l: (df_multi_remaining[l] == 1).sum() for l in disease_labels
            }
            class_counts_multi = {
                k: v for k, v in class_counts_multi.items() if v > 0
            }

            if not class_counts_multi:
                break

            # A. 找出多疾病群中數量最少的標籤 min_label
            min_label = min(class_counts_multi, key=class_counts_multi.get)

            # B. 抽取多疾病群中的 min_label 樣本
            df_multi_min = df_multi_remaining[
                df_multi_remaining[min_label] == 1
            ]
            multi_avail = len(df_multi_min)

            if multi_avail <= still_needed:
                guaranteed_abnormal_indices.update(
                    df_multi_min.index.tolist()
                )
                still_needed -= multi_avail
                print(
                    f"  └ [多疾病群] 稀有標籤 [{min_label}] -> 全抽 {multi_avail} 筆，累積異常: {len(guaranteed_abnormal_indices)}/{n_abnormal_target}"
                )
            else:
                sampled = df_multi_min.sample(
                    n=still_needed, random_state=sampling_seed
                )
                guaranteed_abnormal_indices.update(sampled.index.tolist())
                print(
                    f"  └ [多疾病群] 稀有標籤 [{min_label}] -> 隨機抽取 {still_needed} 筆補滿，達目標額度"
                )
                break

            # C. 檢查該標籤在「單一疾病群」中的剩餘可用數量，決定是否全抽
            df_single_remaining = df_single[
                ~df_single.index.isin(guaranteed_abnormal_indices)
            ]
            df_single_min = df_single_remaining[
                df_single_remaining[min_label] == 1
            ]
            single_count = len(df_single_min)

            if single_count < 100 and single_count > 0:
                print(
                    f"    └ 觸發條件：[{min_label}] 在單一疾病群僅 {single_count} 筆 (< 100) -> 執行全抽策略"
                )
                if single_count <= still_needed:
                    guaranteed_abnormal_indices.update(
                        df_single_min.index.tolist()
                    )
                    print(
                        f"      └ [單一疾病群] [{min_label}] 全抽 {single_count} 筆，累積異常: {len(guaranteed_abnormal_indices)}/{n_abnormal_target}"
                    )
                else:
                    sampled_single = df_single_min.sample(
                        n=still_needed, random_state=sampling_seed
                    )
                    guaranteed_abnormal_indices.update(
                        sampled_single.index.tolist()
                    )
                    print(
                        f"      └ [單一疾病群] [{min_label}] 隨機抽取 {still_needed} 筆補滿，達目標額度"
                    )
                    break
            else:
                print(
                    f"    └ 跳過單一疾病全抽：[{min_label}] 在單一疾病群共有 {single_count} 筆 (>= 100)"
                )

        # 3. 若多疾病抽取完成後仍未達到目標數量，從剩餘單一疾病群隨機補足
        still_needed = n_abnormal_target - len(guaranteed_abnormal_indices)
        if still_needed > 0 and not df_single.empty:
            ...
            if not df_single_remaining.empty:
                ...
                sampled_single = df_single_remaining.sample(
                    n=draw_num, random_state=sampling_seed
                )
                guaranteed_abnormal_indices.update(
                    sampled_single.index.tolist()
                )
                print(
                    f"🍂 [單一疾病群] 名額未滿，從剩餘單一疾病池隨機補充 {draw_num} / {len(df_single_remaining)} 筆"
                )
    ...
    for label in target_labels:
        pos_count = (
            (df_balanced[label] == 1).sum() if label in df_balanced.columns else 0
        )
    ...
    for _, row in tqdm(
        ...
        if raw_label in ["No Finding", "nan", ""] or (
            "No_Finding" in row and row["No_Finding"] == 1
        ):
            ...
            if len(labels_clean) == 1:
                ...
                findings_str = (
                    ", ".join(labels_clean[:-1]) + f" and {labels_clean[-1]}"
                )
        ...
        possible_paths = [
            ...
            os.path.join(
                os.path.dirname(image_root), os.path.basename(pic_str)
            ),
        ]
        ...
        converted_data.append({"messages": messages, "images": [image]})

    if missing_count > 0:
        print(
            f"⚠️ [警告] 共有 {missing_count} 筆影像檔案不存在或無法載入，已自動忽略。"
        )
        if first_failed_path:
            print(
                f"🔍 [除錯訊息] 嘗試讀取的首筆無效路徑為: {first_failed_path}"
            )
